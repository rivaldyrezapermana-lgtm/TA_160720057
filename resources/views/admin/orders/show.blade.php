    ...
    <div class="lg:col-span-2 space-y-6">
            ...
            <table class="table-clean">
                ...
                <tbody>
                    @foreach ($order->items as $i)
                        <tr>
                            <td class="font-medium">{{ $i['product'] }}</td>
                            <td class="text-ink-500">{{ $i['size'] }}</td>
                            <td class="text-right tabular-nums">{{ $i['qty'] }}</td>
                            <td class="text-right tabular-nums">Rp {{ number_format($i['price'], 0, ',', '.') }}</td>
                            <td class="text-right tabular-nums font-medium">Rp {{ number_format($i['subtotal'], 0, ',', '.') }}</td>
                        </tr>
                    @endforeach
                </tbody>
                <tfoot class="border-t-2 border-ink-200">
                    <tr><td colspan="4" class="px-4 py-3 text-right font-semibold">Total</td><td class="px-4 py-3 text-right font-display text-lg font-semibold">Rp {{ number_format($order->total, 0, ',', '.') }}</td></tr>
                </tfoot>
            </table>
        </x-ui.card>

        <x-ui.card title="Pengiriman">
            <p class="text-sm font-medium text-ink-900">{{ $order->customer }}</p>
            <p class="text-sm text-ink-600">{{ $order->phone }} · {{ $order->email }}</p>
            <p class="text-sm text-ink-700 mt-2">{{ $order->shipping_address }}</p>
        </x-ui.card>
        ...
        <x-ui.card title="Pembayaran">
            <div class="grid md:grid-cols-2 gap-4 text-sm">
                ...
                <div><p class="text-ink-500">Bukti Transfer</p>
                    <a href="#" class="text-ink-900 underline text-sm">Lihat bukti</a>
                </div>
                <div><p class="text-ink-500">Tanggal Bayar</p><p class="font-medium">{{ $order->payment['paid_at'] }}</p></div>
            </div>
        </x-ui.card>
    </div>
