...
<div class="max-w-5xl mx-auto px-6 py-10">
    ...
    <div class="flex items-start justify-between mb-8">
        <div>
            ...
            <p class="text-sm text-ink-500 mt-1">Dipesan pada {{ $order->date }}</p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="space-y-3">
                    @foreach ($order->items as $i)
                        <div class="flex gap-4 items-center">
                            <div class="w-16 h-16 bg-ink-100 rounded-lg flex-shrink-0"></div>
        ...
        <div class="space-y-6">
            <div class="bg-white border border-ink-100 rounded-xl p-5">
                ...
                <p class="text-sm text-ink-600">{{ $order->shipping_address }}</p>
            </div>
            ...
            <div class="bg-white border border-ink-100 rounded-xl p-5">
                ...
                <div class="text-sm space-y-1">
                    ...
                    <p class="text-ink-500">Status: <x-ui.status-badge :status="$order->payment['status']" /></p>
                </div>
                @if (!$order->payment['proof'])
                    <form action="{{ route('checkout.proof', $order->id) }}" method="POST" enctype="multipart/form-data" class="mt-3 space-y-2">
                        @csrf
                        <input type="file" name="proof" accept="image/*" class="input text-xs">
